  ...
  const createOrder = async ({ name, phone, email }) => { // Cambia el nombre de la función a `createOrder`
    ...
    try {
      ...
      if (outOfStock.length === 0) {
        ...
        const orderRef = collection(db, 'orders'); // Corrige el nombre de la variable `ordeRef` a `ordersRef`
        const orderAdded = await addDoc(orderRef, objOrder); // Quita `.id` de `orderAdded.id
